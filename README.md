...
4. **雙行事曆支援（Google / Apple）**：
   - **Google 行事曆**：採用官方範本連結格式，自動預填課程、老師與廠館位置。
   - **Apple 行事曆**：iPhone / Safari 深度支援，一鍵喚起 iOS 原生「加入行程」視窗，標題主旨與 Google 行事曆一致（包含課程名稱、授課老師、運動場館），並預設設定上課前 2 小時與 1 小時雙重提醒。
5. **響應式排版 (RWD)**：
