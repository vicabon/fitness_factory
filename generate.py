...
.chip{display:inline-flex;align-items:center;background:#f4f7fb;
      border:1px solid #dde3ed;border-radius:20px;padding:6px 12px;
      font-size:13px;cursor:pointer;transition:all .15s;user-select:none;
      min-height:34px;line-height:1;white-space:nowrap}
.chip:hover{border-color:#1b4f72;background:#e0eaf5}
.chip.on{border-color:#1b4f72;background:#e0eaf5;color:#1b4f72;font-weight:600}
...
function buildCnameGrid() {
  ...
  // click handler: toggle .on then filter
...
// ── store chips（全選/取消全選）────────────────────────────────────────────
document.querySelectorAll('.store-chips .chip').forEach(chip => {
  chip.addEventListener('click', () => {
    chip.classList.toggle('on');
    applyFilter();
  });
