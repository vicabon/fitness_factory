...
.chip{display:inline-flex;align-items:center;gap:4px;background:#f4f7fb;
      ...
      min-height:34px;line-height:1;white-space:nowrap;color:#444}
...
tr.wh td{background:#e8f4fd;color:#154360;font-weight:700;font-size:15px;
          padding:8px 14px;border-top:2px solid #bee3f8;cursor:pointer;
          user-select:none}
tr.wh td:hover{background:#d4ecf7}
tr.wh .wh-arrow{float:right;font-size:13px;transition:transform .2s;color:#5b8db8}
tr.wh.collapsed .wh-arrow{transform:rotate(-90deg)}
tr.day-row.hidden{display:none}
.wd{font-weight:700;color:#1b4f72;white-space:nowrap;font-size:15px}
.dt{font-size:13px;color:#9ab;display:block}
...
.apple-btn{display:inline-flex;align-items:center;gap:4px;
           padding:5px 8px;border-radius:6px;font-size:12px;font-weight:600;
           background:#fdf2f8;border:1.5px solid #f472b6;color:#9d174d;
           text-decoration:none;cursor:pointer;transition:all .15s;white-space:nowrap}
.apple-btn:hover,.apple-btn:active{background:#9d174d;color:#fff;border-color:#9d174d}
.cal-btn-group{display:inline-flex;flex-wrap:wrap;gap:4px;align-items:center}
.card-item .cal-btn-group{margin-top:8px}
.card-item .gcal-btn,.card-item .apple-btn{padding:6px 12px;border-radius:8px}
...
.day-hdr-txt{font-size:16px;font-weight:700;color:#154360;letter-spacing:.3px}
.day-hdr-arrow{font-size:13px;color:#5b8db8;transition:transform .2s}
.day-hdr.collapsed .day-hdr-arrow{transform:rotate(-90deg)}
...
function getGcalParams(c) {
  ...
  if (!m) return null;
  const [,sh,sm,eh,em] = m;
  const d = c.date.replace(/-/g,'');           // YYYYMMDD
  const start = d + 'T' + sh.padStart(2,'0') + sm + '00';
  const end   = d + 'T' + eh.padStart(2,'0') + em + '00';
...
function addToAppleCalendar(cIdx) {
  const c = currentFilteredRows[cIdx];
  if (!c) return;
  const m = c.course_time.match(/(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})/);
  if (!m) { alert('課程時間格式無法解析'); return; }
  const [, sh, sm, eh, em] = m;
  // 與 Google 日曆一致的標頭主旨格式：課程名稱 | 授課老師 @ 運動場館
  const title = c.course_name + ' | ' + c.teacher + ' @ ' + c.store;
  ...
  const ics = [
    ...
    'BEGIN:VALARM',
    'ACTION:DISPLAY',
    'DESCRIPTION:課程提醒（2小時前）',
    'TRIGGER:-PT120M',
    'END:VALARM',
    'BEGIN:VALARM',
    'ACTION:DISPLAY',
    'DESCRIPTION:課程提醒（1小時前）',
    'TRIGGER:-PT60M',
    'END:VALARM',
    'END:VEVENT',
    'END:VCALENDAR',
...
function renderCalButtons(c, index) {
  ...
  return '<div class="cal-btn-group">'
    + '<a class="gcal-btn" href="' + gcUrl + '" target="_blank">&#128197; Google行事曆</a>'
    + '<button type="button" class="apple-btn" onclick="addToAppleCalendar(' + index + ')">&#127822; Apple行事曆</button>'
    + '</div>';
}
...
function toggleDay(hdr) {
  const key  = hdr.dataset.daykey;
  const open = !hdr.classList.contains('collapsed');
  if (open) {
    hdr.classList.add('collapsed');
    // table rows
    document.querySelectorAll('tr.day-row[data-daykey="' + key + '"]')
      .forEach(r => r.classList.add('hidden'));
    // card body
    const body = document.querySelector('.day-body[data-daykey="' + key + '"]');
...
function expandAll() {
  document.querySelectorAll('tr.wh, .day-hdr').forEach(h => h.classList.remove('collapsed'));
  document.querySelectorAll('tr.day-row, .day-body').forEach(r => r.classList.remove('hidden'));
}
function collapseAll() {
  document.querySelectorAll('tr.wh, .day-hdr').forEach(h => h.classList.add('collapsed'));
  document.querySelectorAll('tr.day-row, .day-body').forEach(r => r.classList.add('hidden'));
}
...
<div class="wrap">
  ...
  <div class="rbar">
    ...
    <div style="display:flex;align-items:center;gap:8px">
      <button class="btn bs" style="padding:6px 12px;font-size:12px" onclick="expandAll()">全展開</button>
      <button class="btn bs" style="padding:6px 12px;font-size:12px" onclick="collapseAll()">全摺疊</button>
